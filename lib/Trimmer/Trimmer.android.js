...
export class Trimmer extends Component {
    ...
    return (
      ...
      >
        <View style={styles.row}>
          ...
          <View style={[styles.cornerItem, { position: 'absolute', right: 0, top: 0, bottom: 0, borderTopLeftRadius: 8, borderBottomLeftRadius: 8 }]} />
        </View>
      </Animated.View>
    );
  }
    ...
    return (
      ...
      >
        <View style={[styles.row, { height: 50 }]}>
          <View style={styles.bgBlack} />
          <View style={[styles.cornerItem, { position: 'absolute', left: 0, top: 0, bottom: 0, borderTopRightRadius: 8, borderBottomRightRadius: 8 }]} />
  ...
  renderTopBorder() {
    const { rightCorner, leftCorner } = this.state;
    const right = rightCorner.interpolate({
      inputRange: [-width, 0],
      outputRange: [width + 20, 20],
      extrapolate:'clamp'
    })
    const left = leftCorner.interpolate({
      inputRange: [0, width],
    ...
    return (
      <Animated.View
        style={[{ borderTopWidth: 1, position: 'absolute', top: 0, borderColor: "white" }, { right: right, left: left }]}
        {...this.rightResponder.panHandlers}
      >
      </Animated.View>
    )
  ...
  renderBottomBorder() {
    ...
    return (
      <Animated.View
        style={[{ borderTopWidth: 1, position: 'absolute', top: 49, borderColor: "white" }, { right: right , left: left }]}
        {...this.rightResponder.panHandlers}
      >
      </Animated.View>
    )
    ...
    return (
      <View
        ...
        onLayout={({ nativeEvent }) => {
          if (this.state.layoutWidth != nativeEvent.layout.width) {
            this.setState({
